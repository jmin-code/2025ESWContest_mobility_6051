...
if final_text.strip():
    try:
        print("[INFO] 실시간 스트리밍으로 음성 출력을 시작합니다...")
        
        # 음성 잘림 현상을 막기 위해 텍스트 앞에 ". " 추가
        tts_text = ". " + final_text
        tts = gTTS(text=tts_text, lang='ko')

        # mpg123 프로세스를 파이프 모드로 실행
        ...
        # 음성 데이터를 메모리 버퍼에 쓰고, 바로 mpg123으로 전달
        fp = io.BytesIO()
        tts.write_to_fp(fp)
        fp.seek(0)
        ...
        print("[INFO] 음성 출력이 완료되었습니다.")
    except Exception as e:
        print(f"[ERROR] 음성 출력 중 오류 발생: {e}")
